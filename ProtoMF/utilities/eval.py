...
def NDCG_at_k_batch(logits: np.ndarray, k=10, sum=True):
    ...
    ndcgs = np.zeros(n) # batch size

    if rows.size > 0:
        ndcgs[rows] = 1. / np.log2((cols + 1) + 1) # for the positive rows (correctly ranked) 
    ...
    def eval_batch(self, out: np.ndarray, sum: bool = True):
        ...
        for k in K_VALUES:
            for metric_name, metric in zip(['ndcg@{}', 'hit_ratio@{}'], [NDCG_at_k_batch, Hit_Ratio_at_k_batch]):
                if sum:
                    self.metrics_values[metric_name.format(k)] = self.metrics_values.get(metric_name.format(k), 0) + metric(out, k, True)
                else:
